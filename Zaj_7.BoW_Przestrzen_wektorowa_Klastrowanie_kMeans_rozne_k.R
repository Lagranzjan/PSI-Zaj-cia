...
docs <- DirSource("C:/Users/Karol/Downloads/textfolder")
# W razie potrzeby dostosuj ścieżkę
# np.: docs <- DirSource("C:/User/Documents/textfolder")


# 0. Utwórz korpus dokumentów tekstowych
corpus <- VCorpus(docs)
...
#  1. Normalizacja i usunięcie zbędnych znaków ----
...
# 2. Macierze częstości TDM i DTM ----
...
# 1. Czy na podstawie wyników (otrzymanych klastrów) można wnioskować o treści dokumentów?
# Tak, możliwe jest wnioskowanie o ogólnej treści dokumentów na podstawie wyników klastrowania.
# Analiza najczęściej występujących słów (Top słów) oraz chmur słów w poszczególnych klastrach
# pozwala określić dominujące tematy w każdej grupie dokumentów.
# Należy jednak zaznaczyć, że wnioskowanie to ma charakter przybliżony,
# ponieważ algorytm k-means grupuje dokumenty na podstawie podobieństwa słów,
# a nie rzeczywistego zrozumienia ich treści.

# 2. Jakich ogólnie obszarów tematycznych dotyczą klastry oraz ile jest tych obszarów?
# Obszary tematyczne klastrów można określić na podstawie najczęściej występujących słów
# w każdym klastrze. Każdy klaster reprezentuje jeden obszar tematyczny,
# np. związany z relacjami międzyludzkimi, edukacją, konfliktami lub inną tematyką,
# w zależności od analizowanego zbioru danych.
# Liczba obszarów tematycznych jest równa liczbie klastrów (k),
# która została wcześniej ustalona w procesie analizy (np. k = 2, 3 lub 4).
